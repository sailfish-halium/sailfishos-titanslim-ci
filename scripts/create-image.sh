...
cp $TMPWORKDIR/rpm/usr/share/kickstarts/Jolla-\@RELEASE\@-$DEVICE-*\@ARCH\@.ks $OUTPUTDIR/Jolla-\@RELEASE\@-$DEVICE-$VERSION-\@ARCH\@.ks

# Use home repo instead of nemo:devel:hw
sed -i -e 's|nemo:/devel:/hw:/unihertz|home:/TheKit|g' $OUTPUTDIR/Jolla-\@RELEASE\@-$DEVICE-$VERSION-\@ARCH\@.ks

mount
df -h

echo "Creating mic with $OUTPUTDIR/Jolla-\@RELEASE\@-$DEVICE-$VERSION-\@ARCH\@.ks "
sudo mic create fs --pack-to=sfe-$DEVICE-$RELEASE$EXTRA_NAME.tar.bz2 --arch=$PORT_ARCH \
